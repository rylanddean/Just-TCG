...
- [ ] `CardFilterState` is extended with:
...
- [ ] `isEmpty` on `CardFilterState` returns `true` only when all fields (including new ones) are at their default "no filter" state
- [ ] The predicate/filter logic in `CardsView` and `CardPickerView` is updated to apply all new filters
- [ ] All filters compose with AND logic (a card must satisfy every active filter to appear)
- [ ] Filters for HP and damage treat `nil` bounds as open-ended (`hpMin: 80, hpMax: nil` = HP ≥ 80)
- [ ] `attackingEnergyTypes` filter matches if *any* attack cost contains *any* of the selected types (OR within the set, AND with other filters)
- [ ] `retreatCosts` filter: card's `retreatCost` equals *any* selected value (OR within the set)
- [ ] `weaknessTypes` / `resistanceTypes`: card's single weakness/resistance type is *any* of the selected values
- [ ] Trainer and Energy cards (no HP, no attacks): pass HP/damage/retreat/ability filters transparently unless those filters are narrowly set (e.g. HP min > 0 excludes them naturally)
