...
- [x] A `MatchLogWidget` view appears in the `HomeView` scroll stack above the placeholder row
- [x] The widget has a header row with the title **"Recent Matches"** and a **"Log Match"** button (pill or tappable text) on the trailing edge
- [x] Tapping **"Log Match"** opens `LogMatchSheet` as a sheet — deck selection should default to the most recently used deck if one exists, otherwise prompt for deck selection as normal
- [x] Up to **5 most recent matches** are displayed, ordered by `date` descending, across all decks
- [x] Each match row shows:
...
- [x] If no matches have been logged yet, the widget body shows an empty-state message: **"No matches yet — log your first game."**
- [x] A **"See All"** link at the bottom of the widget navigates to the full match history (can be a `NavigationLink` to a placeholder `MatchHistoryView` if M3-04 is not yet built)
- [x] Widget data refreshes when the sheet dismisses (use `.onDismiss` or observe the model context)
