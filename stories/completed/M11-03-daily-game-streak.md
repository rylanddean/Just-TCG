...
- [x] `StreakEngine` is a `struct` (or `enum` with no cases) with a single static method:
  ```swift
  static func compute(matches: [Match], dailyGoal: Int, calendar: Calendar = .current) -> StreakResult
  ```
- [x] `StreakResult` is a value type with:
...
- [x] A `StreakWidget` view appears in the `HomeView` scroll stack, directly **above** `MatchLogWidget`
- [x] The widget displays:
...
- [x] The daily goal is read from `UserDefaults` with key `"streak_daily_goal"` and defaults to `1`
- [x] `StreakWidget` recomputes on appear and whenever the match list changes (via `@Query`)

### Settings Integration

- [x] A **"Daily game goal"** stepper (range 1–10) is added to `SettingsView` in a new "Streak" section
- [x] Changing the stepper writes to `UserDefaults` key `"streak_daily_goal"` immediately
- [x] The stepper label reads `"Goal: X games / day"`
