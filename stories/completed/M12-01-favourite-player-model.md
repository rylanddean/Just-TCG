...
- [x] A `FavouritePlayer` SwiftData model is added with the following properties:
...
- [x] `FavouritePlayer` is included in the app's `ModelContainer` schema alongside existing models
- [x] A `FavouritePlayerRepository` (`@Observable` class) exposes:
  - `func isFavourite(id: String) -> Bool`
  - `func add(_ player: FavouritePlayer)`
  - `func remove(id: String)`
  - `var all: [FavouritePlayer]` — sorted by `addedAt` descending
- [x] `add` is a no-op if a record with that `id` already exists — no duplicates
- [x] Adding and removing a favourite persists correctly across cold launches
